        ...
        <el-table :data="sortedBlogs" @sort-change="handleSortChange">
          ...
          <el-table-column label="操作" width="250px">
            <template slot-scope="scope">
              ...
              <el-button @click="handleAccess(scope.row)" type="primary" size="small">过审</el-button>
...
<script>
import { getPendingBlogsByPage, drawbackBlog, accessBlog } from '@/api/blog';
...
export default {
  ...
  methods: {
    ...
    async handleAccess(blog) {
      try {
        await accessBlog(blog)
          .then((response) => {
            if (response.data.code === 200) {
              this.$message.success('过审成功');
              this.fetchBlogs();
            } else {
              this.$message.error(response.data.msg);
            }
...
<style lang="less" src="@/assets/css/BlogReview.less" scoped></style>
